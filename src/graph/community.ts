...
import { upsertCommunitySummary, pruneCommunitySummaries, getIncrementalCommunities, getMeta, setMeta } from "../store/store.ts";
...
export type SummaryMode = "incremental" | "full";

/** 全量摘要的最小间隔（毫秒），默认 24 小时 */
const FULL_SUMMARY_INTERVAL_MS = 24 * 60 * 60 * 1000;

/**
 * 为社区生成 LLM 摘要描述 + embedding 向量
 *
 * @param mode - "incremental": 只处理新增+成员变化的社区
 *               "full": 处理所有社区（用于每日全量刷新）
 *               默认 "incremental"
 *
 * 增量逻辑：
 *   - 新增社区（gm_communities 无记录）→ 摘要
 *   - 成员数变化（node_count 不匹配）→ 摘要
 *   - 无变化 → 跳过
 *
 * 全量逻辑：
 *   - 处理所有社区，并记录完成时间到 gm_meta
 */
export async function summarizeCommunities(
  ...
  // 决定要处理的社区子集
  let targetCommunities: Map<string, string[]>;

  if (mode === "full") {
    targetCommunities = communities;
  } else {
    // incremental: 只处理新增 + 成员变化的社区
    targetCommunities = getIncrementalCommunities(db, communities);

    // 自动全量兜底：距上次全量 >24h 则自动切换为 full
    const lastFullStr = getMeta(db, "last_full_summary_at");
    if (lastFullStr) {
      const elapsed = Date.now() - parseInt(lastFullStr, 10);
      if (elapsed > FULL_SUMMARY_INTERVAL_MS) {
        if (process.env.GM_DEBUG) {
          console.log(`  [DEBUG] auto full summary: ${Math.round(elapsed / 3600000)}h since last full`);
        }
        targetCommunities = communities;
        mode = "full";
      }
  ...
  for (const [communityId, memberIds] of targetCommunities) {
  ...
  // 全量模式完成后记录时间戳
  if (mode === "full" && generated > 0) {
    setMeta(db, "last_full_summary_at", String(Date.now()));
  }
